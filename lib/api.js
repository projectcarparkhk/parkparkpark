...
export async function getAllSubDistrictsGroupByArea(locale = 'zh') {
  const data = await client.fetch(`*[_type == 'area']{
    _id, 
    'name': name.${locale},
...
export async function getAllCarParks(slug, preview, locale = 'zh') {
  const data = await client.fetch(`*[_type == 'subDistrict' && slug.${locale}Slug.current == $slug]{
    _id, 
    'name': name.${locale},
    'slug': slug.${locale}Slug.current,
